{{-- File: resources/views/frontend/profile/addresses/edit.blade.php --}}
@extends('layouts.app')
...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="mb-6">
            ...
            <p class="text-gray-600">Update your address information.</p>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <form action="{{ route('profile.addresses.update', $address->id) }}" method="POST" id="address-form">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <div class="flex space-x-4">
                        <label class="flex items-center p-4 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors {{ old('label', $address->label) == 'Kantor' ? 'border-orange-500 bg-orange-50' : '' }}">
                            <input type="radio" 
                                   name="label" 
                                   value="Kantor" 
                                   class="sr-only" 
                                   {{ old('label', $address->label) == 'Kantor' ? 'checked' : '' }}
                                   onchange="updateRadioStyles()">
                            <div class="radio-custom mr-3"></div>
                            <span class="text-sm font-medium text-gray-700">Kantor</span>
                        </label>
                        
                        <label class="flex items-center p-4 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors {{ old('label', $address->label) == 'Rumah' ? 'border-orange-500 bg-orange-50' : '' }}">
                            <input type="radio" 
                                   name="label" 
                                   value="Rumah" 
                                   class="sr-only" 
                                   {{ old('label', $address->label) == 'Rumah' ? 'checked' : '' }}
                                   onchange="updateRadioStyles()">
                            <div class="radio-custom mr-3"></div>
                ...
                <div class="mb-4 p-4 bg-gray-50 border border-gray-200 rounded-md">
                    <h4 class="font-medium text-gray-800 mb-2">Current Location:</h4>
                    <p class="text-sm text-gray-700">{{ $address->location_string }}</p>
                    <button type="button" onclick="showLocationSearch()" class="text-orange-600 hover:text-orange-800 text-sm mt-2">
                        Change Location
                    </button>
                </div>

                <!-- Location Search (Hidden by default) -->
                <div id="location-search-container" class="hidden mb-4">
                    ...
                    <button type="button" onclick="hideLocationSearch()" class="text-gray-600 hover:text-gray-800 text-sm mt-2">
                ...
                <div id="selected-location" class="hidden mb-4 p-4 bg-green-50 border border-green-200 rounded-md">
                    <div class="flex justify-between items-start">
                        <div>
                            <h4 class="font-medium text-green-800">New Selected Location:</h4>
                ...
                <input type="hidden" name="subdistrict_name" id="subdistrict_name" value="{{ $address->subdistrict_name }}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.hideLocationSearch = function() {
        ...
        document.getElementById('subdistrict_name').value = '{{ $address->subdistrict_name }}';
        document.getElementById('postal_code').value = '{{ $address->postal_code }}';
        document.getElementById('destination_id').value = '{{ $address->destination_id }}';
    };

    window.clearLocation = function() {
        // Clear hidden fields (will require new selection)
